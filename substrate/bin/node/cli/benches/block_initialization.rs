...
fn prepare_benchmark(client: &FullClient, block_number: BlockNumber, block_hash: Hash) -> (usize, Vec<OpaqueExtrinsic>) {
...
fn block_initialization(c: &mut Criterion) {
	...
	let (max_transfer_count, extrinsics) = prepare_benchmark(&client, best_number, best_hash);
